...
function add_bm($new_url) {
  ...
  // check not a repeat util
  $result = $conn->query("select * from util
  ...
  // insert the new util
  if (!$conn->query("insert into util values
     ('".$valid_user."', '".$new_url."')")) {
...
function delete_bm($user, $url) {
  ...
  // delete the util
  if (!$conn->query("delete from util where
...
function recommend_urls($valid_user, $popularity = 1) {
  ...
  if (!($result = $conn->query($query))) {
     throw new Exception('Could not find any utils to recommend.');
  }
